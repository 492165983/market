          ...
          <div class="tabsContent">
            <div id=minuteFive class="tableMinuteContent" v-show='MinuteMenuIndex ==  0'>
              <table class="tableOne">
                <!-- 卖五 -->
                <tbody>
                  <tr v-for='(item,i) in Sell' :key="i">
                    <td>卖{{-i + 5}}</td>
                    <td class="color-change" style="color:red">{{item.price}}</td>
                    <td>{{item.vol}}</td>
                  </tr>
                </tbody>
              </table>
              <table class="tableTwo">
                <!-- 买五 v-bind:class="item.color"-->
                <tbody>
            ...
            <!-- <div id="minute" class="tableMinuteContent" v-show='MinuteMenuIndex == 1'>
              <table class="tableInfo">
                <tbody>
                  <tr v-for='(item,index) in StockData.Dealer' :key="index">
                    <td>{{item.timer}}</td>
    ...
    RequestMinuteData (data, callback, option) {
      ...
      $.ajax({
        ...
        success: (recvData) => {
          ...
          this.Sell = Sell.reverse()  //左边的this是全局的,右边是变量
        },
        error: () => {
          // console.log('错误');
        }
