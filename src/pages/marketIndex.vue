<template>
  <div class="hqWrap">
    ...
    <div class="horizontal" @click="GotoHorizontal()">
      <span style="font-size:22px;" class="hq_iconfont icon-quanpingzuidahua"></span>
    </div>
    ...
    <div v-show="MinuteTooltip.IsshowTooltip && Minute.IsShow" class="tabList kline-tip">
      ...
      价 <span class="tooltip-span" :class="MinuteTooltip.Data.Increase.Color">{{MinuteTooltip.Data.Close.Text}}</span>
      幅 <span class="tooltip-span" :class="MinuteTooltip.Data.Increase.Color">{{MinuteTooltip.Data.Increase.Text}}%</span>
      量 <span class="tooltip-span" :class="MinuteTooltip.Data.Increase.Color">{{MinuteTooltip.Data.Vol.Text}}</span>
      额 <span class="tooltip-span" :class="MinuteTooltip.Data.Increase.Color">{{MinuteTooltip.Data.Amount.Text}}</span>
    </div>
    <!--k线部分-->
...
import '../../node_modules/hqchart/src/jscommon/umychart.resource/js/webfont'
...
export default {
  components: { TabList, subIndex, changeSymbol },
  name: "marketIndex",
  inject: ['reload'],
  ...
  data () {
    return {
      ...
      preclose_px: 0,    // 昨收价(非资信)   
      hqChart: {},   // 分时线的
      hqChart1: {},   // 分时图 资信指数的
      hqChartKline: {},  //k线的
      hqChartKline3: {},  //k线的  大盘资信指数
      hqChartKline1: {},  //1分钟k线的
      hqChartKline2: {},  //1分钟k线的  大盘资信指数
      hqChartBS: {}, // BS 数据
  ...
  methods: {
    OnSize () {
      ...
      var chartHeight = window.innerHeight - window.innerHeight * (0.5);
    ...
    //切换 导航栏    现在暂时无效
    ChangeKlinIndex (indexItem, flag) {
    ...
    CreateMinuteChart () {
      ...
      blackStyle.UnchagneBarColor = "#DC143C";   //设置平盘颜色
      // blackStyle.FrameSplitPen = "#ECECEC";  //分割线
      // blackStyle.FrameBorderPen = '#ECECEC';  // 边框
    ...
    MinuteNetworkFilter (data, callback) {
      // console.log(this);
      // console.log('[MinuteNetworkFilter] data', data, callback);
      switch (data.Name) {
    ...
    NetworkFilter (data, callback) {
      switch (data.Name) {
        ...
        case 'KLineChartContainer::RequestHistoryData':                 //日线全量数据下载
          this.RequestHistoryData(data, callback, { PageSize: 1000 });
    ...
    ReqeustHistoryMinuteData (data, callback, option) {
      ...
      var symbol = data.Request.Data.symbol; //股票代码
      var name = data.Name;
      // console.log(window.myProdCode);
      console.log(window.myProdCodeBYSJ);
      if (window.myProdCodeBYSJ === undefined) {
        $.ajax({
          type: 'GET',
          url:
    ...
    JsonToHQChartMinuteHistoryData (recvData) {
      ...
      for (var key in obj) {
        ...
        this.preclose_px = obj[key].pre_close_px;
...
.exchangeData .priceHL .low,
.exchangeData .priceOpen .change,
.exchangeData .priceAmount .totalValue {
  height: 20px;
  line-height: 20px;
}
...
.exchangeData .priceHL span,
.exchangeData .priceOpen span,
.exchangeData .priceAmount span {
  float: right;
}
...
.tabsContent #minuteFive .tableOne tbody > tr {
  height: 50px;
}
.tabsContent #minuteFive .tableTwo tbody > tr {
  height: 50px;
}
.kLineTabs .tabsTitle {
  padding-left: 2%;
...
.minuteWrap #minuteChart,
.minuteWrap #minuteFiveDaychart,
.minuteWrap #kline {
  float: left;
  overflow: hidden;
}
...
.tableOne tr > td:nth-of-type(1),
.tableTwo tr > td:nth-of-type(1),
.tableInfo tr > td:nth-of-type(1) {
  width: 26%;
  color: #333333;
}
.tableOne tr > td:nth-of-type(2),
.tableTwo tr > td:nth-of-type(2),
.tableInfo tr > td:nth-of-type(2) {
  width: 30%;
  text-align: center;
}
.tableOne tr > td:nth-of-type(3),
.tableTwo tr > td:nth-of-type(3),
.tableInfo tr > td:nth-of-type(3) {
  width: 37%;
  text-align: center;
  color: #333333;
}
...
.qAList .qText {
  color: #f39800;
  font: 1.071rem "PingFang-SC-Regular";
}
.qAList .aText {
  margin-top: 9px;
  color: #217cd9;
...
.qAList .time,
.qAList .time > span {
  font: 0.857rem "PingFang-SC-Regular";
  color: #8b8b8b;
}
.qAList .time > .author {
  margin-right: 9.6%;
